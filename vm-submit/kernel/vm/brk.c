...
do_brk(void *addr, void **ret)
{
       ...
       // decide ending and starting of brk
       // whether the range is empty between start and end brk
       // set the mappings if not page alligned.(This page will have read/write permissions).
       // put value of new break into ret.
       if(addr == NULL){
               *ret = curproc->p_brk;
       ...
       uint32_t npages = (((uint32_t)addr-(uint32_t)curproc->p_brk))/PAGE_SIZE;
       uint32_t end_vfn = ((uint32_t) addr - 1) / PAGE_SIZE;
       end_vfn+=1;
       vmmap_t* curr_map = curproc->p_vmmap;
       uint32_t start_vfn = ((uint32_t) curproc->p_brk - 1) / PAGE_SIZE;
       start_vfn+=1;
//        if(((uint32_t)addr > (uint32_t)curproc->p_brk)){
        if(start_vfn < end_vfn){
               // create heap
               if(vmmap_is_range_empty(curr_map,start_vfn,end_vfn-start_vfn) != 1){
                  dbg(DBG_PRINT,"(GRADING3E)\n");
                  return -ENOMEM;
               }else{
                        vmarea_t* vma = vmmap_lookup(curr_map,start_vfn-1);
                        dbg(DBG_PRINT,"(GRADING3B)\n");
                        vma->vma_end = end_vfn;
               }
               dbg(DBG_PRINT,"(GRADING3B)\n");
       }else if(start_vfn > end_vfn) {
               // destroy heap
               vmmap_remove(curr_map,end_vfn,-(end_vfn-start_vfn));
               dbg(DBG_PRINT,"(GRADING3E)\n");
       }
       dbg(DBG_PRINT,"(GRADING3B)\n");
       curproc->p_brk = addr;
