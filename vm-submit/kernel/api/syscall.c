...
sys_read(read_args_t *arg)
{
        ...
        int err = 0;
        if ((err = copy_from_user(&kread_args, arg, sizeof(kread_args))) < 0) {
                dbg(DBG_PRINT,"GRADING3B\n");
                curthr->kt_errno = -err;
                return -1;
        }
        void* buffer = page_alloc();
       if ((err = do_read(kread_args.fd, buffer, kread_args.nbytes)) < 0) {
               dbg(DBG_PRINT,"GRADING3D 1\n");
               curthr->kt_errno = -err;
               page_free(buffer);
               return -1;
        ...
        dbg(DBG_PRINT,"GRADING3D 1\n");
        int read_success = 0;
        if ((read_success = copy_to_user(arg->buf, buffer, err)) < 0) {
                dbg(DBG_PRINT,"GRADING3D 1 err\n");
                curthr->kt_errno = -read_success;
                page_free(buffer);
                return -1;
        }
        page_free(buffer);
...
sys_write(write_args_t *arg)
{
        ...
        int err = 0;
        if ((err = copy_from_user(&kwrite_args, arg, sizeof(kwrite_args))) < 0) {
                dbg(DBG_PRINT,"GRADING3B\n");
                curthr->kt_errno = -err;
                return -1;
        }
        void* buffer = page_alloc();
        int write_success = 0;
        if ((write_success = copy_from_user(buffer, kwrite_args.buf, kwrite_args.nbytes)) < 0) {
                dbg(DBG_PRINT,"GRADING3D err\n");
                curthr->kt_errno = -write_success;
                page_free(buffer);
                return -1;
        }
       if ((write_success = do_write(kwrite_args.fd, buffer, kwrite_args.nbytes)) < 0) {
               dbg(DBG_PRINT,"GRADING3D 1\n");
               curthr->kt_errno = -write_success;
               page_free(buffer);
               return -1;
...
sys_getdents(getdents_args_t *arg)
{
        ...
        getdents_args_t kgetdent_args;
        int err = 0;
        if ((err = copy_from_user(&kgetdent_args, arg, sizeof(kgetdent_args))) < 0) {
                dbg(DBG_PRINT,"GRADING3B\n");
                curthr->kt_errno = -err;
                return -1;
        }
        int ret = 0;
        dirent_t d_t;
        int count = (int)kgetdent_args.count;
        int num_of_dirents = count / sizeof(dirent_t);

        
        for(int i =0;i<num_of_dirents;i++){
               int curr = 0;
               if((curr = do_getdent(kgetdent_args.fd, &d_t))==0){
                       dbg(DBG_PRINT,"GRADING3B\n");
                       curthr->kt_errno = 0;
                       break;
               }else if(curr<0){
               ...
               dbg(DBG_PRINT,"GRADING3B\n");
               int err = 0;
             if((err = copy_to_user(kgetdent_args.dirp + i,&d_t,curr)) <0){
                     dbg(DBG_PRINT,"GRADING3D err\n");
                     curthr->kt_errno = -err;
                     return -1;
             }
