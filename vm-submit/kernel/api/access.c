...
addr_perm(struct proc *p, const void *vaddr, int perm)
{
        ...
        vmarea_t *vma = vmmap_lookup(p->p_vmmap,vpn);
        if(vma==NULL){
                dbg(DBG_PRINT,"addr_perm (GRADING 3D)\n");
                return 0;
        }
        dbg(DBG_PRINT,"addr_perm (GRADING 3B)\n");
        return 1;
}
...
range_perm(struct proc *p, const void *avaddr, size_t len, int perm)
{
       // NOT_YET_IMPLEMENTED("VM: range_perm");
        uint32_t diff = 0;
        void* curr_vaddr = NULL;
        while(diff<len){
                uint32_t addr = (uint32_t)avaddr + diff;
                curr_vaddr = (void *) addr;
                int ret = addr_perm(p,curr_vaddr,perm);
                if(ret == 0){
                        dbg(DBG_PRINT,"range_perm (GRADING 3D)\n");
                        return ret;
                }
                dbg(DBG_PRINT,"range_perm (GRADING 3B)\n");
                diff = diff+PAGE_SIZE;
        }
        dbg(DBG_PRINT,"range_perm (GRADING 3B)\n");
        return 1;
